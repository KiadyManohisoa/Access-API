...
class Utilisateur
{
    ...
    public function se_connecter(Connection $connection): ?Compte {
        ...
        try {
            ...
            if (!$result) {
                throw new MailIntrouvableException("Le mail {$this->mail} est introuvable.");
            }
            ...
            if (!password_verify($this->mdp, $mdpHash)) {
                throw new CompteException("Le mot de passe est incorrect.");
            }

            // Vérifier si le compte est bloqué
            $currentDate = new \DateTime();
            if ($dateDebloquage && $currentDate <= $dateDebloquage) {
                throw new CompteBloqueeException("Le compte est bloqué jusqu'au {$dateDebloquage->format('Y-m-d H:i:s')}.");
            }

            // Si tout est valide, instancier l'objet Compte
            $compte->id = $id;
            $compte->utilisateur = $this;
            $compte->tentative = new Tentative((int)$result['d_nb_tentative'], $dateDebloquage);
            $compte->pin = new Pin($result['d_pin_actuel'], new \DateTime($result['d_date_expiration_pin']));
        } catch (\Exception $e) {
            // Rethrow l'exception pour la gestion en dehors de cette méthode
            throw $e;
        }
